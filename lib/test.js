...
function loadResources() {
    ...
    .then((rows) => {
      ...
      rows.forEach(function(row) {
       ...
       promises.push(b);
       let c = knex.select('id', 'tags.name').from('tags').join('resource_tags', 'resource_tags.tag_id', 'tags.id').where('resource_tags.resource_id', row.id).then((tags)=>{
          row.tags = tags;
       })
       promises.push(c);
       let d = knex.select('id').from('likes').where({resource_id:row.id, user_id:request.session.user_id}).then((likes)=>{
         row.likes = likes;
       })
        promises.push(d);
     })
